...
| Analytics | `Analytics.test.tsx`; `reporting/router.test.ts` | Aggregate endpoint totals are not asserted by the client test | No list envelope; one daily/volume trend fixture | N/A — charts and KPIs | 1 each: `sale.aggregate`, `sale.groupBy`, `saleLine.groupBy`, `sale.count`, and Hamper lookup; 0 `sale.findMany` |
