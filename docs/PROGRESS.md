...
|------|-------|------|--------|--------|
| 2026-08-19 | Claude + reviewers | PR #42 review fixes (label clearing, refresh semantics, restore fallback, DRY helpers) | Done | codex/editable-settings |
| 2026-08-19 | Codex + subagents | Editable settings, archive/restore, and audit history | Done | codex/editable-settings |
...
| 2026-08-21 | Codex + subagents | PR #43 review follow-up: inventory totals and active category counts | Done | codex/site-pagination-filter-performance |
...
| 2026-08-14 | Codex | Design manual Etsy Sale resolution and verification-status filtering | Done | codex/manual-etsy-sale-resolution |
| 2026-08-14 | Codex | Plan manual Etsy Sale resolution implementation | Done | codex/manual-etsy-sale-resolution |
| 2026-08-14 | Codex subagent | Task 1: Persist and contract manual Etsy Sale resolution state | Done | codex/manual-etsy-sale-resolution |
| 2026-08-14 | Codex subagent | Task 2: Calculate penny-exact receipt-group resolutions | Done | codex/manual-etsy-sale-resolution |
...
| 2026-08-14 | Codex subagent | Task 5: Filter Sales and summaries by verification status | Done | codex/manual-etsy-sale-resolution |
| 2026-08-14 | Codex subagent | Task 5 fix round 1: Guard stale Sales filter and pagination responses | Done | codex/manual-etsy-sale-resolution |
| 2026-08-14 | Codex subagent | Task 6: Build guarded Sales resolution modal | Done | codex/manual-etsy-sale-resolution |
| 2026-08-14 | Codex subagent | Task 6 fix round 1: preserve loaded expansion and modal focus management | Done | codex/manual-etsy-sale-resolution |
...
- **PR #43 review follow-up complete** (branch: `codex/site-pagination-filter-performance`): Inventory's Total Inventory card now uses full server-filtered aggregates instead of current-page sums. The existing filtered CTE computes unit quantity and bulk positive-lot totals before pagination without adding a normal-page database query; empty and out-of-range pages use the existing bounded fallback query. Category badges now count active products only, matching the expanded soft-delete-filtered list. RED regressions reproduced both findings; focused Inventory/category server tests (19) and client tests (30) pass. A real PGlite regression executes mixed-unit totals across 26 filtered products, two pages, and an out-of-range page. Independent review approved the final candidate after that database-level coverage was added. No schema migration, production database, external API, or data write was used.
- **Latest `origin/main` integrated and Prisma enum compatibility restored** (branch: `codex/site-pagination-filter-performance`): merged upstream commit `c7dd1dc` while preserving numbered Sales pagination, server-side filtering/sorting, retained-result updates, stale-request protection, and the newer manual Etsy resolution flow. A pre-merge regression proved the stale branch contracts rejected `MANUAL` / `MANUALLY_VERIFIED`; after Prisma Client generation, the paginated margin-sort contract accepts both values. Independent review found no remaining integration or simplify issues after checking the accepted URL-backed `page` / `pageSize` scope. Full client/server tests, both TypeScript projects, the production build, focused ESLint, Prisma schema validation/generation, and diff checks pass. No migration, production database, Etsy call, push, or external write was performed. Report: `.superpowers/sdd/2026-08-20-site-pagination-filter-performance/task-11-report.md`.
...
- **PR #42 review fixes complete (branch: `codex/editable-settings`):** six findings scored 50+ were fixed. Behaviour: (1) an Add with a blank/omitted postage-tier label now clears the stored label instead of silently keeping the old one, since Prisma skips `label: undefined` on update; (2) post-mutation refreshes route through `refreshAfterMutation`, so a failed reload surfaces its own banner rather than making an already-committed mutation look failed — this stopped a packaging-overhead retry from inserting a duplicate row (`PackagingOverhead.name` has no unique constraint, so duplicates double-count in `totalPerOrder`); (3) supplier Add falls back to the read snapshot when the row vanishes mid-restore instead of returning a spurious 404. DRY: `isPrismaError` moved to `server/lib/prismaError.ts`, three drifting `getErrorDetails` copies collapsed into `src/lib/apiError.ts`, and duplicated `withArchived` moved to `src/lib/api/query.ts`. Verification: full suite **990/990** passing (68 files, 1 pre-existing `Sales.test.tsx` env failure requiring Supabase vars), `tsc -b` clean, ESLint clean on all touched files.
- **Editable settings complete (Task 8, branch: `codex/editable-settings`):** the root cause was archived unique values being inserted instead of restored or updated; editable fields, archive/restore surfaces, and latest-first Audit History with expandable redacted snapshots are now shipped. Migration `20260819103000_add_settings_audit_log` records each mutation in the same transaction as the setting write, preserving audit atomicity. Etsy Access Management remained untouched. Final independent review approved the branch with no remaining Critical or Important findings. Fresh verification passed Prisma generation/validation with dummy localhost URLs, focused server **63/63**, focused client **127/127**, full server **293/293**, full client **585/585**, both project TypeScript checks, touched-file ESLint, production build, and `git diff --check`. Rollout: apply the Prisma migration before deploying the server.
...
- **Etsy resolution modal simplify cleanup complete** (branch: `codex/manual-etsy-sale-resolution`): apply-time locking is centralized on one layout-neutral outer fieldset; nested resolution controls no longer repeat the same lock, while fee inputs retain their attribution-only disable rule. The focused modal suite passes 15/15, `rtk tsc -p tsconfig.json --noEmit` passes, focused ESLint reports 0 errors, and `rtk git diff --check` passes. No test changes, database, Etsy, statement, Payment, migration, or external write was used.
- **Manual Etsy Sale resolution final review fixes complete** (branch: `codex/manual-etsy-sale-resolution`): ID-only authoritative-evidence failures now use an exact typed 409 conflict; suffix-row manual verification normalizes the receipt base; mounted Etsy reconciliation counts refresh through the owning hook; post-commit refresh failures cannot relabel a committed apply as failed; and apply locks modal state with an in-flight guard. Deferred tests cover stale pending previews, suffix rows, visible count refresh, post-commit failure semantics, and double apply. Focused server tests pass 36/36, focused client tests pass 81/81, full server tests pass 23 files/331 tests, full client tests pass 38 files/582 tests, both TypeScript checks and the 1,195-module build pass, and focused ESLint has 0 errors with the 2 existing `SalesPage` hook warnings. No database, Etsy, statement, Payment, migration, or external write was used. Report: `.superpowers/sdd/2026-08-14-manual-etsy-sale-resolution/final-fix-report.md`.
- **Manual Etsy Sale resolution Task 1 complete** (branch: `codex/manual-etsy-sale-resolution`): added the additive manual-verification migration and Prisma fields, shared resolution request/apply/preview contracts with integer-pence fields and safe receipt IDs, manual status/source labels, reconciliation summary count, and the verification filter foundation. Focused contract tests pass 7/7, fee-route tests pass 16/16, full server tests pass 20 files/285 tests, focused client tests pass 63/63, full client tests pass 37 files/549 tests, Prisma validation/generation, both TypeScript checks, production build, touched-file ESLint, and `git diff --check` pass. No database, Etsy service, statement upload, or production migration was used. Report: `.superpowers/sdd/2026-08-14-manual-etsy-sale-resolution/task-1-report.md`.
- **Manual Etsy Sale resolution Task 2 complete** (branch: `codex/manual-etsy-sale-resolution`): added the pure penny-exact receipt-group calculator with exact/immediate-suffix identity validation, collision/evidence/immutability guards, deterministic gross-weight fee/VAT allocation, final-balance manual verification, channel reclassification formulas, and BigInt safe-range checks. Focused calculations tests pass 15/15, full server tests pass 21 files/300 tests, server TypeScript, touched-file ESLint, and `git diff --check` pass. No database, Etsy service, statement upload, or production migration was used. Report: `.superpowers/sdd/2026-08-14-manual-etsy-sale-resolution/task-2-report.md`.
- **Manual Etsy Sale resolution Task 2 fix round 1 complete** (branch: `codex/manual-etsy-sale-resolution`): calculator inputs, BigInt intermediates, and every proposal write field now enforce the Prisma `Decimal(10,2)` pence range of ±9,999,999,999, including boundary and derived net/margin overflow regressions. Focused calculations tests pass 17/17, server TypeScript, focused ESLint, and `git diff --check` pass. No database, Etsy service, statement upload, or production migration was used.
- **Manual Etsy Sale resolution Task 3 complete** (branch: `codex/manual-etsy-sale-resolution`): added a fingerprinted preview/apply service, exact Prisma Decimal-to-pence adapter, id+updatedAt compare-and-set transaction writes, typed conflict/validation/not-found errors, and actual Sales router endpoints before `GET /:id`. Service and route tests pass 14/14; the full server suite passes 23 files/316 tests; server TypeScript, touched-file ESLint, and `git diff --check` pass. RED was observed before each new service/route implementation. No database, Etsy service, statement upload, production migration, or external write was used. Report: `.superpowers/sdd/2026-08-14-manual-etsy-sale-resolution/task-3-report.md`.
- **Manual Etsy Sale resolution Task 3 fix round 1 complete** (branch: `codex/manual-etsy-sale-resolution`): Prisma apply now revalidates the exact/immediate-suffix receipt group inside a `Serializable` transaction before any CAS update, rejecting phantom membership changes; controlled adapter tests cover the ±9,999,999,999 pence Decimal boundaries, corrected destination-group membership, and two-row rollback after a later CAS conflict. Focused service/router tests pass 18/18; the full server suite passes 23 files/320 tests; server TypeScript, touched-file ESLint, and `git diff --check` pass. No database, Etsy service, statement upload, production migration, or external write was used. Report: `.superpowers/sdd/2026-08-14-manual-etsy-sale-resolution/task-3-report.md`.
- **Manual Etsy Sale resolution implementation plan complete** (branch: `codex/manual-etsy-sale-resolution`): seven serialized TDD/review tasks cover additive persistence/contracts, penny-exact receipt-group calculations, fingerprinted atomic API writes, statement/Payment authority, exact/combined Sales status filtering, the guarded resolution modal, and disposable migration/full compatibility verification. Plan: `docs/superpowers/plans/2026-08-14-manual-etsy-sale-resolution.md`. No application code, migration, database, or Etsy state changed during planning.
- **Manual Etsy Sale resolution design approved** (branch: `codex/manual-etsy-sale-resolution`): expanded unresolved Etsy Sale details will provide a fingerprint-guarded Preview → Confirm modal for Direct/Fair reclassification, safe receipt-ID correction, and exact final manual Offsite fee/VAT verification across complete receipt groups. Statement-verified Sales remain immutable; explicit statements may supersede manual verification, while Payment may not. The Sales screen will gain exact/combined reconciliation-status filters applied consistently to list and summary, and the Etsy reconciliation summary will count Etsy Sales only. Design: `docs/superpowers/specs/2026-08-14-manual-etsy-sale-resolution-design.md`. Tasks 1 and 2 are complete; later implementation tasks remain.
- **Manual Etsy Sale resolution Task 5 complete** (branch: `codex/manual-etsy-sale-resolution`): Sales list/count and summary data/unverified counts now share a typed verification-status predicate; invalid status queries return 400; combined `needs_verification` covers PENDING, PAYMENT_SYNCED, and MANUAL_REVIEW; Etsy reconciliation counts are Etsy-only; and the Sales UI forwards one filter to list and summary reloads. Focused server tests pass 23/23 and focused client tests pass 44/44; both TypeScript checks, touched-file ESLint (0 errors), and `git diff --check` pass. Existing React `act(...)` and hook-dependency warnings remain; no database, Etsy, statement, Payment, or external writes were used. Report: `.superpowers/sdd/2026-08-14-manual-etsy-sale-resolution/task-5-report.md`.
- **Manual Etsy Sale resolution Task 5 fix round 1 complete** (branch: `codex/manual-etsy-sale-resolution`): Sales list/summary and pagination responses now carry a shared request-generation guard; stale filter responses cannot replace rows, summaries, errors, or loading state, and filter reloads invalidate pending Load More results. Deferred-response regressions cover rapid status changes and Load More followed by a status change. Focused Sales tests pass 33/33, client TypeScript and focused ESLint pass (0 errors; 2 existing hook-dependency warnings), and `git diff --check` passes. No database, Etsy, statement, Payment, or external writes were used. Report: `.superpowers/sdd/2026-08-14-manual-etsy-sale-resolution/task-5-report.md`.
- **Manual Etsy Sale resolution Task 6 complete** (branch: `codex/manual-etsy-sale-resolution`): added the local-form guarded Preview → Confirm Sales resolution modal with exact BigInt pence conversion, server-only preview totals, stale-preview invalidation, 400/409 handling, and double-submit guards; wired visibility to unresolved Etsy rows; refreshed filtered Sales list/summary, Etsy status counts, and expanded detail after apply; and displayed the optional manual note. Focused client tests pass 64/64, full client tests pass 38 files/576 tests, full server tests pass 23 files/330 tests, both TypeScript checks, build, touched-file ESLint (0 errors; existing SalesPage hook warnings), and `git diff --check` pass. Report: `.superpowers/sdd/2026-08-14-manual-etsy-sale-resolution/task-6-report.md`.
- **Manual Etsy Sale resolution Task 6 fix round 1 complete** (branch: `codex/manual-etsy-sale-resolution`): guarded resolution refreshes now retain loaded pages so an expanded sale beyond the first page remains visible when it still matches active filters; refreshed detail is dropped and expansion cleared when it no longer matches. The modal now focuses its initial Close control, traps Tab/Shift+Tab within the dialog, and restores the opener focus. Focused modal/page tests pass 50/50, full client tests pass 38 files/578 tests, client TypeScript, production build, focused ESLint (0 errors; 2 existing SalesPage hook-dependency warnings), and `git diff --check` pass. No database, Etsy, statement, Payment, or external writes were used. Report: `.superpowers/sdd/2026-08-14-manual-etsy-sale-resolution/task-6-report.md`.
- **Manual Etsy Sale resolution Task 7 complete** (branch: `codex/manual-etsy-sale-resolution`): reran the exact-name PostgreSQL 16 `tmpfs` migration exercise and checked in the literal seed, before/after, migration, enum/column, and assertion SQL plus psql outputs; all 19 prior migrations applied, three representative rows retained `sales=3`, `fees=10.00`, `netRevenue=140.00`, and `margin=77.00`, and new enum values/`etsyManualResolutionNote` were verified. Focused suites pass 8 files/129 server tests and 3 files/66 client tests; complete suites pass 23 files/330 server tests and 38 files/578 client tests; Prisma generation/validation, both TypeScript checks, build, and `git diff --check` pass. The mandated unchanged `rtk eslint ...` command now passes with a process-local prefix of the repository `node_modules/.bin` (0 errors, 2 existing SalesPage warnings); existing test warnings are recorded in the report. The exact container was removed and verified absent; no production database, Etsy, statement, Payment, migration, or external write was used. Transcript/report: `.superpowers/sdd/2026-08-14-manual-etsy-sale-resolution/task-7-report.md`.
...
- `$env:VITE_SUPABASE_URL='http://localhost'; $env:VITE_SUPABASE_ANON_KEY='test-anon-key'; npm run test:client:run -- src/__tests__/components/EtsyFeeReconciliationPanel.test.tsx src/__tests__/components/EtsyOrdersSyncPanel.test.tsx src/__tests__/lib/api/etsy.test.ts` — PASS, 3 files / 63 tests (dummy values; existing React `act(...)` warnings only).
- `npx tsc -p server/tsconfig.json --noEmit --rootDir .`, `npx tsc -p tsconfig.json --noEmit`, `npm run build`, and `npx eslint server/lib/etsy/fees/statementParser.ts server/lib/etsy/fees/paymentNormalizer.ts server/lib/etsy/fees/paymentReconciliation.ts server/__tests__/etsy/statementParser.test.ts server/__tests__/etsy/paymentReconciliation.test.ts src/features/etsy/components/EtsyFeeReconciliationPanel.tsx src/__tests__/components/EtsyFeeReconciliationPanel.test.tsx` — PASS; `git diff --check` PASS with existing LF-to-CRLF conversion warnings. The reviewed pre-existing `EtsySyncPanel.test.tsx:163` Location-assignment error was not reproduced by the current TypeScript toolchain.
- `npm run test:server:run` — PASS, 19 files / 239 tests; existing Etsy env warnings, expected unique-constraint test message, and mock-mode notices.
- `$env:VITE_SUPABASE_URL='http://localhost'; $env:VITE_SUPABASE_ANON_KEY='test-anon-key'; npm run test:client:run` — PASS, 37 files / 549 tests; existing React `act(...)`, mocked API/network/camera, and Recharts zero-size warnings.
- No database, Etsy service, production system, or downloaded CSV was accessed.
