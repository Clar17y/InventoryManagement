...
**Current State:**
- **Etsy Offsite Ads implementation complete** (branch: `codex/etsy-offsite-fee-reconciliation`): Tasks 1–10 are complete, including the guarded UI, reporting, and safe historical rollout runbook. No production database, disposable database, or Etsy account was accessed; no migration, backfill, Payment apply, or statement apply was run.
...
- **Task 10 complete** (branch: `codex/etsy-offsite-fee-reconciliation`): added `docs/ETSY_OFFSITE_FEE_RUNBOOK.md` with backup, disabled-gate, two-receipt Payment validation prerequisite, chronological statement preview/apply, review-ID capture, monthly comparison, explicit approval, stale/duplicate/manual-review handling, rollback, and the no-production-apply boundary. The runbook explicitly records the current blocker: shipped Payment UI/API do not expose normalized gross/fees/net, so `ETSY_PAYMENT_FEES_VALIDATED` must remain false until a controlled read-only diagnostic/feature exists. README links the runbook/design and documents the disabled validation flag. No live Etsy or database data was accessed or changed.
...
- Migration/data-preservation boundary — `npx prisma migrate diff --from-empty --to-schema-datamodel prisma/schema.prisma --script` generated SQL without a database; the two reconciliation migration files were inspected for status-only backfill and no monetary updates. The disposable before/after SQL queries are recorded in the runbook but were not executed because this task's boundary forbids database apply/access.
...
- `npx tsc -p server/tsconfig.json --noEmit --rootDir .` — PASS. `npx tsc -p tsconfig.json --noEmit` — BLOCKED by the pre-existing `src/__tests__/components/EtsySyncPanel.test.tsx:163` `window.location` assignment type error.
...
- Operational blocker — the shipped Payment preview UI/API do not expose normalized Payment gross, aggregate fees, or net. The two-receipt gate validation cannot be completed from the shipped surfaces; keep `ETSY_PAYMENT_FEES_VALIDATED=false` until a controlled read-only diagnostic/feature exposes and verifies those values. This is deferred operational work, not a Task 10 code change.
