...
| 2026-08-12 | Codex subagent | Task 10 review follow-up: isolated disposable migration-preservation verification | In Progress | codex/etsy-offsite-fee-reconciliation |
...
**Current State:**
- **Etsy Offsite Ads implementation** (branch: `codex/etsy-offsite-fee-reconciliation`): Tasks 1–9 and the Task 10 documentation are complete; the isolated disposable migration-preservation verification is In Progress. No production database, disposable database, or Etsy account was accessed; no migration, backfill, Payment apply, or statement apply was run.
...
- **Task 10 documentation complete; migration verification pending** (branch: `codex/etsy-offsite-fee-reconciliation`): added `docs/ETSY_OFFSITE_FEE_RUNBOOK.md` with backup/PITR gate, disabled-gate, two-receipt Payment validation prerequisite, chronological statement preview/apply, review-ID capture, monthly baseline-plus-delta formulas, explicit authorization, stale/duplicate/manual-review handling, rollback, and the no-production-action boundary. The runbook explicitly records the current blocker: shipped Payment UI/API do not expose normalized gross/fees/net, so `ETSY_PAYMENT_FEES_VALIDATED` must remain false until a controlled read-only diagnostic/feature exists. README links the runbook/design and documents the disabled validation flag. The unique disposable migration-preservation check is tracked as In Progress; no live Etsy or database data was accessed or changed.
