...
- `npm run test:server:run -- server/__tests__/etsy/statementParser.test.ts server/__tests__/etsy/paymentReconciliation.test.ts server/__tests__/etsy/feeRoutes.test.ts` — PASS, 3 files / 62 tests; existing missing `ETSY_API_KEY`/`ETSY_SHARED_SECRET` warnings only.
...
- `npm run test:server:run` — PASS, 19 files / 237 tests; existing Etsy env warnings, expected unique-constraint test message, and mock-mode notices.
