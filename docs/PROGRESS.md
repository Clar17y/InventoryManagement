...
**Current State:**
- **Editable settings complete (Task 8, branch: `codex/editable-settings`):** the root cause was archived unique values being inserted instead of restored or updated; editable fields, archive/restore surfaces, and latest-first Audit History with expandable redacted snapshots are now shipped. Migration `20260819103000_add_settings_audit_log` records each mutation in the same transaction as the setting write, preserving audit atomicity. Etsy Access Management remained untouched. Available verification: focused Audit History and Settings page tests, **2 files / 36 tests**, both project TypeScript checks, Prisma generation/validation with dummy localhost URLs, touched-file ESLint, production build, and diff check all pass; full suites are queued for fresh final verification. Rollout: apply the Prisma migration before deploying the server.
