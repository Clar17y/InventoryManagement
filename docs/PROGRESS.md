...
- `npm run test:server:run -- server/__tests__/etsy/statementParser.test.ts server/__tests__/etsy/paymentReconciliation.test.ts server/__tests__/etsy/feeRoutes.test.ts` — PASS, 3 files / 61 tests; existing missing `ETSY_API_KEY`/`ETSY_SHARED_SECRET` warnings only.
- `$env:VITE_SUPABASE_URL='http://localhost'; $env:VITE_SUPABASE_ANON_KEY='test-anon-key'; npm run test:client:run -- src/__tests__/components/EtsyFeeReconciliationPanel.test.tsx src/__tests__/components/EtsyOrdersSyncPanel.test.tsx src/__tests__/lib/api/etsy.test.ts` — PASS, 3 files / 59 tests (dummy values).
- `npx tsc -p server/tsconfig.json --noEmit --rootDir .`, `npx tsc -p tsconfig.json --noEmit`, `npm run build`, and `npx eslint server/lib/etsy/fees/statementParser.ts server/lib/etsy/fees/paymentNormalizer.ts server/lib/etsy/fees/paymentReconciliation.ts server/__tests__/etsy/statementParser.test.ts server/__tests__/etsy/paymentReconciliation.test.ts src/features/etsy/components/EtsyFeeReconciliationPanel.tsx src/__tests__/components/EtsyFeeReconciliationPanel.test.tsx` — PASS; `git diff --check` PASS with existing LF-to-CRLF conversion warnings.
- `npm run test:server:run` — PASS, 19 files / 236 tests; existing Etsy env warnings, expected unique-constraint test message, and mock-mode notices.
- `$env:VITE_SUPABASE_URL='http://localhost'; $env:VITE_SUPABASE_ANON_KEY='test-anon-key'; npm run test:client:run` — PASS, 37 files / 545 tests; existing React `act(...)`, mocked API/network/camera, and Recharts zero-size warnings.
